...
Since we can only use TCP and DNS withing Proxychains, we must disable all other protocols used in Host Discovery phase. According to the Nmap Network Scanning Guide:
...
So if we want to skip any UDP or ICMP packets in host discovery and stick only to TCP probes, it should be enough to add the -PT option. This should orverrule all the other ping types.

~~~
proxychains nmap -PT 216.58.222.14 -vv
...
But no luck this time! In fact, we can check our host is probing the target directly:

~~~
$ sudo tcpdump host 216.58.214.228
...
~~~
# proxychains nmap -Pn -p80 -sT 216.58.222.14 -vv
ProxyChains-3.1 (http://proxychains.sf.net)
...
Let's see what happens if we scan the IP address directly, instead of the name:
...
We have been using so far the infamous -sT (SYN scan). What about -sS?
